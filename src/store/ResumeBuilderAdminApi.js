...
const resumeBuilderAdminApi = axios.create({
    baseURL: import.meta.env.VITE_API_RESUMEBUILDERADMIN,
});

const formDataURL = ['user/user-profile/change-avatar'];

resumeBuilderAdminApi.interceptors.request.use((req) => {
    let userTokenData;
    try {
...
resumeBuilderAdminApi.interceptors.response.use(
    (response) => response,
    (error) => {
        if (error.response && [401, 403].includes(error.response.status)) {
            sessionStorage.removeItem('linkedInScrapeToken');
...
export default resumeBuilderAdminApi;
